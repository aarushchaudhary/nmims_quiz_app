...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($sap_ids) && empty($errors)) {
        header('Location: ' . get_base_url() . 'views/admin/manage_elective.php?id=' . $elective_id . '&error=' . urlencode("No SAP IDs provided."));
        exit();
    }

    $stmt_find_student = $pdo->prepare("SELECT user_id FROM students WHERE sap_id = ?");
    $stmt_add_student = $pdo->prepare("INSERT IGNORE INTO elective_students (elective_id, student_id) VALUES (?, ?)");

    foreach ($sap_ids as $sap_id) {
        $stmt_find_student->execute([$sap_id]);
        $student = $stmt_find_student->fetch();
        if ($student) {
            $stmt_add_student->execute([$elective_id, $student['user_id']]);
    ...
    header('Location: ' . get_base_url() . 'views/admin/manage_elective.php?id=' . $elective_id . '&success=' . urlencode($msg));
    exit();
} else {
    header('Location: ' . get_base_url() . 'views/admin/electives.php');
}
