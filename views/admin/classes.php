...
<div class="manage-container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <h2>All Classes (<?php echo count($batches); ?>)</h2>
    </div>
    ...
    <div class="section-box">
        ...
        <form action="<?= get_base_url() ?>api/admin/add_class.php" method="POST" id="addClassForm" style="display:flex; flex-direction:column; gap: 20px;">
            ...
            <div id="batch-rows-container" style="display:flex; flex-direction:column; gap: 15px;">
                <div class="batch-row" style="display:flex; gap: 15px; align-items: flex-start; background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #e5e7eb; position: relative;">
                    <div style="flex: 1;">
                        <label style="font-size: 13px; font-weight: bold; margin-bottom: 5px; display: block;">Section Name</label>
                        <input type="text" name="section_name[]" class="input-field" placeholder="e.g., Section A (or C1)" required style="margin-bottom:0;">
                    </div>
            ...
            <div style="display:flex; justify-content: space-between; align-items: center;">
                <button type="button" id="btn-add-row" style="background: #e5f9f0; color: #28a745; border: 1px solid #28a745; padding: 10px 15px; border-radius: 8px; font-weight: bold; cursor: pointer;">+ Add Another Section</button>
                <button type="submit" class="button-red" style="width:auto; padding: 12px 30px;">Save All Sections</button>
            </div>
        </form>
    </div>
            ...
            <?php foreach ($batches as $batch): ?>
                <tr>
                    ...
                    <td class="action-buttons" style="flex-direction:row;">
                        <a href="edit_class.php?id=<?php echo $batch['id']; ?>" class="btn-edit" style="background-color:#ffc107; color:black; border:none; padding: 8px 12px; font-size: 14px; border-radius: 6px; cursor:pointer; text-decoration:none; margin-right: 5px;">Edit</a>
                        <a href="<?= get_base_url() ?>api/admin/delete_class.php?id=<?php echo $batch['id']; ?>" class="btn-delete" onclick="return confirm('Are you sure you want to delete this section?')" style="background-color:#dc3545; color:white; border:none; padding: 8px 12px; font-size: 14px; border-radius: 6px; cursor:pointer; text-decoration:none;">Delete</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
<script>
    ...
    schoolSelect.addEventListener('change', function() {
        ...
        classSelect.innerHTML = '<option value="">Select Batch</option>';
    ...
    btnAddRow.addEventListener('click', function() {
        ...
        row.innerHTML = `
            <div style="flex: 1;">
                <label style="font-size: 13px; font-weight: bold; margin-bottom: 5px; display: block;">Section Name</label>
                <input type="text" name="section_name[]" class="input-field" placeholder="e.g., Section B (or C2)" required style="margin-bottom:0;">
            </div>
            <div class="autocomplete-container" style="flex: 1.5;">
                <label style="font-size: 13px; font-weight: bold; margin-bottom: 5px; display: block;">Start SAP ID</label>
